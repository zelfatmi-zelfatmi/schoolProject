...
const actions = [
    {
        ...
        primary: true,
        icon: PlusIcon,
        href: route(routes.users.secretary.create),
    },
];
...
const handleConfirmedDelete = async () => {
    ...
    await axios
        ...
        .then(() => {
            ...
            router.visit(route(routes.users.secretary.list, { status: UserStatusEnum.ACTIVE.id }), {
                method: 'get',
            });
        })
        .finally(() => {
...
const breadcumb = [{ name: 'Utilisateurs' }, { name: 'Secrétaires' }];
</script>

<template>
    <Page :actions="actions" :breadcumb="breadcumb" title="Secrétaires" width="lg">
        <Box sectionned>
            <div>
                <IndexFilters
                        ...
                        (item) => [
                            {
                                ...
                                success: true,
                                full: true,
                                href: route(routes.users.secretary.edit, item?.id || '-'),
                            },
        ...
        >
            Etes-vous sûr que vous voulez {{ isRestore ? 'restore' : 'archiver' }} le Secrétaire
