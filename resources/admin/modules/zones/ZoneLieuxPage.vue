...
import { LieuForm, LieuZipItem } from './partials';
...
const headings = [
    { name: 'Lieu', className: 'text-start px-3 font-normal py-2' },
...
const handleConfirmedDelete = () => {
    deleting.value = true;
    router.delete(route(routes.configuration.zones.lieux.destroy, confirmation.value?.id), {
        onFinish: () => {
...
<template>
    <Page title="Lieux" :subtitle="`Zone: ${zone.name}`" width="md" back>
        <Box sectionned :separated="false" class="mt-8">
            ...
            <Table :headings="headings" :is-loading="false" :items="lieux">
                <template #items="{ items }">
                    <LieuZipItem v-for="item in items" :key="item.id" :item="item" @item:delete="confirmation = item" />
                </template>
            </Table>
        </Box>
        ...
        >
            Etes-vous sûr que vous voulez archiver le lieu
